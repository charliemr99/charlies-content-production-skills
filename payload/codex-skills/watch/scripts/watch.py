...
def _prepend_user_bins_to_path() -> None:
    paths = [
        Path.home() / ".local" / "bin",
        Path.home() / "Library" / "Python" / f"{sys.version_info.major}.{sys.version_info.minor}" / "bin",
    ]
    existing = os.environ.get("PATH", "")
    entries = [str(path) for path in paths if path.exists()]
    os.environ["PATH"] = os.pathsep.join([*entries, existing]) if entries else existing


_prepend_user_bins_to_path()
